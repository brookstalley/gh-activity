...
def _search_commits_chunked(
    ...
    if total_count <= MAX_SEARCH_RESULTS:
        ...
        page = 2
        while len(accumulator) < total_count and page <= 10:
...
def _fetch_search_page(
    ...
    query = f"author:{username}+committer-date:{since.isoformat()}..{until.isoformat()}"
    ...
    for item in data.get("items", []):
        ...
        commit_date = committer_date[:10] if committer_date else ""
        commits.append({
            "sha": item.get("sha", ""),
            "repo": repo_name,
            "date": commit_date,
