...
from gh_activity.cli import parse_args, main


class TestParseArgs:
    def test_defaults(self):
        args = parse_args([])
        assert args.granularity == "week"
        assert args.output == "gh-activity-report.html"
        assert args.refresh is False
        assert args.username is None
        assert args.until == date.today()
        assert args.since == date.today() - timedelta(days=365)
...
class TestMain:
    ...
    def test_full_pipeline_with_username(
        ...
        mock_search.return_value = [
            {"sha": "a", "repo": "r", "date": "2025-06-15", "message": "m"},
        ]
    ...
    def test_refresh_ignores_cache(
        ...
        mock_load.return_value = {
            "commits": [{"sha": "old", "repo": "r", "date": "2025-06-15", "message": "m"}],
            "fetched_ranges": [["2025-06-01", "2025-06-30"]],
        }
        mock_search.return_value = [
            {"sha": "new", "repo": "r", "date": "2025-06-15", "message": "m"},
        ]
